...
public class MultiTrajectoryObsDataset extends TypedDatasetImpl implements TrajectoryObsDataset {
   ...
   */
  public void setTrajectoryInfo(Dimension trajDim, Variable trajVar, Dimension timeDim, Variable timeVar,
      Variable latVar, Variable lonVar, Variable elevVar) throws IOException {
    ...
    for (Iterator it = this.netcdfDataset.getRootGroup().getVariables().iterator(); it.hasNext();) {
      ...
      // !curVar.equals( this.timeVar) && //
          !curVar.equals(this.latVar) && !curVar.equals(this.lonVar) && !curVar.equals(this.elevVar)
    ...
    if (this.timeVar.getDataType().equals(DataType.DOUBLE)) {
      ...
      String tmpMsg = "Time var <" + this.timeVar.getFullName() + "> is not a double, float, or integer <"
          + timeVar.getDataType().toString() + ">.";
      // log.error( tmpMsg );
    ...
    for (int i = 0; i < trajArray.getSize(); i++) {
      ...
      if (this.trajVar.getDataType().equals(DataType.STRING)) {
        ...
        String tmpMsg = "Trajectory var <" + this.trajVar.getFullName()
            + "> is not a string, double, float, or integer <" + this.trajVar.getDataType().toString() + ">.";
        // log.error( tmpMsg );
        throw new IllegalStateException(tmpMsg);
      }

      MultiTrajectory curTraj = new MultiTrajectory(curTrajId, i, trajectoryNumPoint, startDate, endDate, this.trajVar,
          this.timeVar, timeVarUnitsString, this.latVar, this.lonVar, this.elevVar, dataVariables, trajectoryVarsMap);
  ...
  private class MultiTrajectory implements TrajectoryObsDatatype {
    ...
    private MultiTrajectory(String id, int idNum, int numPoints, Date startDate, Date endDate, Variable trajVar,
        Variable timeVar, String timeVarUnitsString, Variable latVar, Variable lonVar, Variable elevVar, List variables,
    ...
    public double getTimeValue(int point) throws IOException {
      ...
      try {
        ...
        IllegalArgumentException iae = new IllegalArgumentException(
            "Point <" + point + "> not in valid range <0, " + (this.getNumberPoints() - 1) + ">: " + e.getMessage());
        iae.initCause(e);
        throw iae;
      }
      if (array instanceof ArrayDouble) {
        ...
        throw new IOException(
            "Time variable not float, double, or integer <" + array.getElementType().toString() + ">.");
      }
    }
    ...
    public Range getFullRange() {
      ...
      try {
        ...
        IllegalStateException ise = new IllegalStateException(
            "Full trajectory range invalid <0, " + (this.getNumberPoints() - 1) + ">: " + e.getMessage());
        ise.initCause(e);
        throw (ise);
      }
      return range;
    ...
    public Range getPointRange(int point) throws InvalidRangeException {
      if (point >= this.getNumberPoints())
        throw new InvalidRangeException(
            "Point <" + point + "> not in acceptible range <0, " + (this.getNumberPoints() - 1) + ">.");
    ...
    public Range getRange(int start, int end, int stride) throws InvalidRangeException {
      if (end >= this.getNumberPoints())
        throw new InvalidRangeException(
            "End point <" + end + "> not in acceptible range <0, " + (this.getNumberPoints() - 1) + ">.");
      ...
      for (IndexIterator it = a.getIndexIterator(); it.hasNext();) {
        if (elevVar.getDataType() == DataType.DOUBLE) {
          ...
          throw new IllegalStateException(
              "Elevation variable type <" + elevVar.getDataType().toString() + "> not double, float, int, or long.");
        }
      }
      return a;
    }
