 ...
 * <ol>
 * <li>the station lat, lon, and desc are in the record variable, forcing us to read one obd data for
...
public class MadisStationObsDataset extends StationObsDatasetImpl implements TypedDatasetFactoryIF {
  ...
  public static boolean isValidFile(NetcdfFile ds) {
    ...
    if ((att != null) && att.getStringValue().equals("MADIS ACARS data"))
      return false;
  ...
  public MadisStationObsDataset(NetcdfDataset ds) throws IOException {
    ...
    for (int stnIndex = 0; stnIndex < n; stnIndex++) {
      ...
      if (stn != null) {
        if (debugLinks) {
          parseInfo.append("Already have=" + stationId + " for station at index " + lastValue + " lastRecord= "
              + stn.lastRecord + "\n");
          parseInfo.append(
              "  old lat=" + stn.getLatitude() + " lon= " + stn.getLongitude() + " alt= " + stn.getAltitude() + "\n");
    ...
    if (debug)
      System.out.println("total stations " + stations.size() + " should be = " + n);
  ...
  private class MadisStationImpl extends StationImpl {
    ...
    private MadisStationImpl(String name, String desc, double lat, double lon, double alt, int lastRecord,
        int inventory) {
