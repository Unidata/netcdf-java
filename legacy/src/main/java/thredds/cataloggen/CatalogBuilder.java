...
public interface CatalogBuilder {
   ...
   * @return the CrawlableDataset for the given path or null if the path is not allowed by this CatalogBuilder either
   *         due to filtering or due to the path not being a descendant (or self) of the collection level path.
   ...
   * @throws IllegalArgumentException if the given path has no corresponding proxy dataset handler or its parent dataset
   *         is not an allowed.
