 ...
 *
 * <p>
 * Here we'll look at the parameters used to construct a
 ...
 *
 * <p>
 * Following are the details on how the resulting InvDataset and
 ...
 *
 * <p>
 * See {@link DatasetScanCatalogBuilder} for more details on how a
 * THREDDS server config file (catalog.xml) and the contained datasetScan
 * elements map into CollectionLevelScanner.
 *
 * <a name="multiLevel"></a>
 * <h4>Multi-level Catalogs</h4>
 *
 * <p>
 * Resulting single level catalogs can be used to construct multi-level
 ...
 * catalogs is supported in several ways:
 * </p>
 *
 * <ul>
 ...
 *
 * <p>
 * NOTE: The {@link StandardCatalogBuilder} class is an example of using
...
public class CollectionLevelScanner {
   ...
   *
   * <p>
   * The collectionLevel and catalogLevel parameters are used to properly
   ...
   * </ul>
   * <p>
   * The currentLevel parameter indicates what level is to be scanned. It
   ...
   * described above. Only the location of the datasets is changed.)
   * </p>
   *
   * @param collectionPath the path of the collection, used as the base of all resulting dataset@urlPath values (may be
   *        "", if null, "" is used).
   ...
   * @param currentLevel the location, at or below the catalog level, which is to be scanned for datasets. Only
   *        necessary when multiple catalogs are to be aggregated. May be null. If null, assumed to be same as catalog
   *        level.
   ...
   */
  public CollectionLevelScanner(String collectionPath, CrawlableDataset collectionLevel, CrawlableDataset catalogLevel,
      CrawlableDataset currentLevel, CrawlableDatasetFilter filter, InvService service) {
    ...
    // if ( CrawlableDatasetAlias.isAlias( collectionLevel.getPath() ) ) throw new IllegalArgumentException( "The
    // collection root <" + collectionLevel.getPath() + "> must not be a CrawlableDatasetAlias.");
    ...
    if (!catalogLevel.getPath().startsWith(collectionLevel.getPath()))
      throw new IllegalArgumentException("The catalog root <" + catalogLevel.getPath()
          + "> must be under the collection root <" + collectionLevel.getPath() + ">.");
    if (!collectionLevel.isCollection())
      throw new IllegalArgumentException(
          "The collection root <" + collectionLevel.getPath() + "> must be a collection.");
    ...
    if (currentLevel != null) {
      ...
      if (!currentLevel.getPath().startsWith(catalogLevel.getPath()))
        throw new IllegalArgumentException("The current root <" + currentLevel.getPath()
            + "> must be under the catalog root <" + catalogLevel.getPath() + ">.");
  ...
  // throw new IllegalArgumentException( "The accessPath <" + accessPath.getPath() + "> must start with the
  // accessPathHeader <" + accessPathHeader.getPath() + ">." );
  // if ( ! accessPathHeader.isCollection() )
  // throw new IllegalArgumentException( "The accessPathHeader <" + accessPathHeader.getPath() + "> must be a
  // collection." );
  ...
  public void scan() throws IOException {
    ...
    // @todo Should we move sort to end of this method? As is, we can't use naming or enhancements to determine sort
    // order.
    if (sorter != null)
  ...
  private InvCatalogImpl createSkeletonCatalog(CrawlableDataset topCrDs) {
    ...
    // InvService service = new InvService( this.service.getName(), this.service.getServiceType().toString(),
    // this.service.getBase(), this.service.getSuffix(), this.service.getDescription() );
  ...
  private InvDatasetImpl createInvDatasetFromCrawlableDataset(CrawlableDataset crawlableDs, InvDatasetImpl parentInvDs,
      InvService service) {
    ...
    if (crawlableDs.isCollection()) {
      ...
      curInvDs = new InvDatasetImpl(parentInvDs, newName, null, (service != null ? service.getName() : null),
  ...
  // - baseID + "/" + node.getPath().substring( collectionLevel.length) [when ID added at catalog construction using
  // node info]
  ...
  // - collectionPath + "/" + node.getPath().substring( collectionLevel.length) [when service is absolute (i.e.,
