...
public class DirectoryScanner {
   ...
   */
  public DirectoryScanner(InvService service, String serviceTitle, File serviceBaseUrlDir, String prefixPath,
    ...
    // log.debug( "DirectoryScanner(): service="+service.getName()+"; serviceTitle="+serviceTitle+";
    // serviceBaseUrlDir="+serviceBaseUrlDir+"; createCatalogRefs="+createCatalogRefs);
    // this.serviceTitle = serviceTitle;
    // this.serviceBaseUrlDir = serviceBaseUrlDir;
    // if ( this.serviceBaseUrlDir.getPath().indexOf( "*") == -1)
    // {
    // if ( ! this.serviceBaseUrlDir.isDirectory()) throw new IllegalArgumentException( "Base URL directory is not a
    // directory <" + serviceBaseUrlDir.getAbsolutePath() + ">.");
    // }
   ...
   * @throws IllegalArgumentException if the given directory is not actually a directory or if it is not under the
   *         service base URL directory.
   */
  public InvCatalog getDirCatalog(File directory, String filterPattern, boolean sortInIncreasingOrder,
      boolean addDatasetSize) {
    return (this.getDirCatalog(directory, filterPattern, sortInIncreasingOrder, null, addDatasetSize, null, null,
        null));
  }

  public InvCatalog getDirCatalog(File directory, String filterPattern, boolean sortInIncreasingOrder, String addIdBase,
      boolean addDatasetSize, String dsNameMatchPattern, String startTimeSubstitutionPattern, String duration) {
    ...
    return getDirCatalog(catalogCrDs, filterPattern, sortInIncreasingOrder, addIdBase, addDatasetSize,
        dsNameMatchPattern, startTimeSubstitutionPattern, duration);
  }

  public InvCatalog getDirCatalog(CrawlableDataset catalogCrDs, String filterPattern, boolean sortInIncreasingOrder,
      String addIdBase, boolean addDatasetSize, String dsNameMatchPattern, String startTimeSubstitutionPattern,
      String duration) {
    ...
    if (filterPattern != null) {
      ...
      MultiSelectorFilter.Selector selector =
          new MultiSelectorFilter.Selector(new RegExpMatchOnNameFilter(filterPattern), true, true, false);
    ...
    if (dsNameMatchPattern != null && startTimeSubstitutionPattern != null && duration != null) {
      ...
      enhancerList.add(RegExpAndDurationTimeCoverageEnhancer.getInstanceToMatchOnDatasetName(dsNameMatchPattern,
          startTimeSubstitutionPattern, duration));
    }
    CatalogBuilder catBuilder = new StandardCatalogBuilder(prefixPath, null, collectionCrDs, filter, service, addIdBase,
        null, null, addDatasetSize, new LexigraphicByNameSorter(sortInIncreasingOrder), null, enhancerList, null,
    ...
    // sortInIncreasingOrder=" + sortInIncreasingOrder + "; addIdBase="+addIdBase+"; dsNameMatchPattern=" +
    // dsNameMatchPattern + "; startTimeSubstitutionPattern=" + startTimeSubstitutionPattern + "; duration=" + duration
    // );
    ...
    // && duration != null)
    // {
    // dsSource.addDatasetEnhancer( DatasetEnhancer1.createAddTimeCoverageEnhancer( dsNameMatchPattern,
    // startTimeSubstitutionPattern, duration));
    // }
    //
    // if ( addIdBase != null)
    ...
    // catch ( IOException e )
    // {
    // throw new IllegalArgumentException( "Given directory is not a collection dataset <" + directory.getAbsolutePath()
    // + ">: " + e.getMessage() );
    // }
  ...
  // public InvCatalog getDirCatalog( String dirPattern, String filterPattern, boolean sortInIncreasingOrder, boolean
  // addDatasetSize )
  // {
  // return( this.getDirCatalog( dirPattern, filterPattern, sortInIncreasingOrder, null, addDatasetSize, null, null,
  // null) );
  // }

  // public InvCatalog getDirCatalog( String dirPattern, String filterPattern, boolean sortInIncreasingOrder, String
  // addIdBase, boolean addDatasetSize, String dsNameMatchPattern, String startTimeSubstitutionPattern, String duration
  // )
  // {
  // // If no wildcards in directory pattern, return catalog for single directory.
  // if ( dirPattern.indexOf( "*") == -1) return( this.getDirCatalog( new File( dirPattern), filterPattern,
  // sortInIncreasingOrder, addIdBase, addDatasetSize, dsNameMatchPattern, startTimeSubstitutionPattern, duration));
  //
  // //
  ...
  // log.debug( "dirPattern <" + dirPattern + ">=<" + pathName1 + "[" + index1 + "]" + pathNameWC + "[" + index2 + "]" +
  // pathName2 + ">");
  //
  ...
  // String curMatchPathName = pathName2 == null ? curMatchFiles[i].getPath() : curMatchFiles[i].getPath() + "/" +
  // pathName2;
  ...
  // InvCatalog curCatalog = this.getDirCatalog( new File( curMatchPath), filterPattern, sortInIncreasingOrder,
