...
public class DodsDirDatasetSource extends DatasetSource {
  ...
  protected InvCatalog createSkeletonCatalog(String prefixUrlPath) throws IOException {
    ...
    if (!apString.startsWith(aphString))
      throw new IOException(
          "The accessPoint <" + apString + "> must start with the accessPointHeader <" + aphString + ">.");
    ...
    if (apVersionResultContent.indexOf("DODS") == -1 && apVersionResultContent.indexOf("OPeNDAP") == -1
  ...
  protected List expandThisLevel(InvDataset dataset, String prefixUrlPath) {
    ...
    try {
      ...
      log.warn("expandThisLevel(): IOException while extracting dataset info from given OPeNDAP directory <"
          + dataset.getName() + ">, return empty list: " + e.getMessage());
      return (dsList);
    }
    ...
    for (Iterator it = possibleDsList.iterator(); it.hasNext();) // @todo curDsUrlString = (String) it.next())
    {
      ...
      if (!curDsUrlString.startsWith(this.accessPointHeaderUri.toString())) {
        log.debug("expandThisLevel(): current path <" + curDsUrlString + "> not child of given" + " location <"
            + this.accessPointHeaderUri.toString() + ">, skip.");
        continue;
      }
      ...
      try {
        ...
        log.error("expandThisLevel(): Skipping dataset  <" + curDsUrlString + "> due to URISyntaxException: "
            + e.getMessage());
        continue;
      }
      ...
      try {
        ...
        log.warn("expandThisLevel(): skipping dataset <" + curDsUri.toString() + ">, not under accessPointHeader: "
            + e.getMessage());
        continue;
      }
  ...
  private class DodsDirInvDataset extends InvDatasetImpl {
    ...
    DodsDirInvDataset(InvDataset parent, URI uri) throws IOException {
      ...
      if (dsAbsolutePath.startsWith(accessPointHeaderUri.toString())) {
        ...
        throw new IOException("URI <" + dsAbsolutePath + "> not under accessPointHeader directory <"
            + accessPointHeaderUri.toString() + ">.");
      }
