...
abstract public class DatasetSource {
  ...
  public static final DatasetSource newDatasetSource(String name, DatasetSourceType type,
      DatasetSourceStructure structure, String accessPoint, ResultService resultService) {
    if (type == null) {
  ...
  public String toString() {
    ...
    tmp.append("DatasetSource[name:<" + this.getName() + "> type:<" + this.getType() + "> structure:<"
        + this.getStructure() + "> accessPoint:<" + this.getAccessPoint() + "> and children - " + "ResultService("
        + this.getResultService().getName() + ") - " + "DatasetNamer(" + this.getDatasetNamerList().size() + ") - "
  ...
  public InvDataset expand() throws IOException {
    ...
    if (!this.isCollection(this.accessPointDataset)) {
      String tmpMsg =
          "The access point dataset <" + this.accessPointDataset.getName() + "> must be a collection dataset.";
   ...
   * @param accessPoints
   * @return the top-level dataset
   * @throws IOException if the main access point for this DatasetSource is not a container dataset or any of the given
   *         access points do not exist or are not under the main access point.
   */
  ...
  private void expandRecursive(InvDataset collectionDataset) {
    ...
    for (Iterator i = listAllDatasets.iterator(); i.hasNext();) {
      ...
      if (!DatasetFilter.acceptDatasetByFilterGroup(this.getDatasetFilterList(), curChildDs,
          this.isCollection(curChildDs)))
        continue;
  ...
  private void expandRecursiveCollection(InvDataset collectionDataset, InvDataset childDs) {
    ...
    // Otherwise, add current dataset as a catalogRef.
      ...
      String docName = (childDs.getName() == null || childDs.getName().equals("")) ? "catalog.xml"
   ...
   * @throws IOException if the dataset location does not correspond to an actual dataset or is not below the
  ...
  private void sortDatasets(InvDataset dataset) {
    if (this.getDatasetSorter() == null) {
      ...
      DatasetSorter defaultSorter = new DatasetSorter(new java.util.Comparator() {
  ...
  public boolean equals(Object o) {
    ...
    if (createCatalogRefs != datasetSource.createCatalogRefs)
      return false;
    // if ( expandCatalogRef != datasetSource.expandCatalogRef ) return false;
    if (flatten != datasetSource.flatten)
      return false;
    if (accessPoint != null ? !accessPoint.equals(datasetSource.accessPoint) : datasetSource.accessPoint != null)
      return false;
    if (datasetFilterList != null ? !datasetFilterList.equals(datasetSource.datasetFilterList)
        : datasetSource.datasetFilterList != null)
      return false;
    if (datasetNamerList != null ? !datasetNamerList.equals(datasetSource.datasetNamerList)
        : datasetSource.datasetNamerList != null)
      return false;
    if (name != null ? !name.equals(datasetSource.name) : datasetSource.name != null)
      return false;
    if (resultService != null ? !resultService.equals(datasetSource.resultService)
        : datasetSource.resultService != null)
      return false;
    if (type != null ? !type.equals(datasetSource.type) : datasetSource.type != null)
      return false;
