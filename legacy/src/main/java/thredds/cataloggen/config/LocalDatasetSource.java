...
public class LocalDatasetSource extends DatasetSource {
  ...
  protected InvCatalog createSkeletonCatalog(String prefixUrlPath) throws IOException {
    ...
    InvService service = new InvService(this.getResultService().getName(),
        this.getResultService().getServiceType().toString(), this.getResultService().getBase(),
        this.getResultService().getSuffix(), this.getResultService().getDescription());
  ...
  private class LocalInvDataset extends InvDatasetImpl {
    ...
    LocalInvDataset(InvDataset parent, File file, String prefixUrlPath) throws IOException {
      ...
      if (logger.isDebugEnabled())
        logger.debug(
            "LocalInvDataset(): parent=" + (parent == null ? "" : parent.getName()) + "; file=" + file.getPath());
      ...
      // NOTE: Started using File.getCanonicalFile() to deal with comparisons containing this (".") or parent ("..")
      // directories.
      // But that caused caused comparison problems for symbolic links (e.g., on motherlode
      // "/usr/local/apache/htdocs/motherlode/dods/model"
      // does not start with "/usr/local/apache/htdocs/motherlode" because "model" is a link to
      // "/data/ldm/pub/decoded/netcdf/GRIB").
      if (!uriStringFromFile.startsWith(aphUriStringFromFile)) {
        if (!uriStringFromCanonicalFile.startsWith(aphUriStringFromCanonicalFile)) {
          String tmpMsg = "File <" + uriStringFromFile + " or " + uriStringFromCanonicalFile
              + "> must start with accessPointHeader <" + aphUriStringFromFile + " or " + aphUriStringFromCanonicalFile
              + ">.";
          throw new IOException(tmpMsg);
        } else {
      ...
      if (!this.directory) {
        ...
        if (LocalDatasetSource.this.getResultService().getBase().equals("")
          ...
          String tmpUrlPath =
              prefixUrlPath == null || prefixUrlPath.equals("") ? relativePath : prefixUrlPath + "/" + relativePath;
      ...
      String dsName = relativePath.endsWith("/") ? relativePath.substring(0, relativePath.length() - 1) : relativePath;
