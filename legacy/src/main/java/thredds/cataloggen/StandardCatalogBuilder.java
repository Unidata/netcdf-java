...
public class StandardCatalogBuilder implements CatalogBuilder {
  ...
  public StandardCatalogBuilder(String collectionPath, String collectionName, CrawlableDataset collectionCrDs,
      CrawlableDatasetFilter filter, InvService service, String collectionId, CrawlableDatasetLabeler identifier,
      CrawlableDatasetLabeler namer, boolean doAddDatasetSize, CrawlableDatasetSorter sorter, Map proxyDatasetHandlers,
      List childEnhancerList, InvDatasetImpl topLevelMetadataContainer, CatalogRefExpander catalogRefExpander) {
    ...
    if (catalogRefExpander != null) {
      ...
      while (catRefInfoList.size() > 0) {
        ...
        if (catalogRefExpander.expandCatalogRef(curCatRefInfo)) {
          ...
          CollectionLevelScanner curScanner = new CollectionLevelScanner(collectionPath, collectionCrDs, catalogCrDs,
              curCatRefInfo.getCrawlableDataset(), filter, service);
  ...
  public InvCatalogImpl generateProxyDsResolverCatalog(CrawlableDataset catalogCrDs, ProxyDatasetHandler pdh)
    ...
    if (!proxyDatasetHandlers.containsValue(pdh))
  ...
  private CollectionLevelScanner setupAndScan(CrawlableDataset catalogCrDs, CrawlableDataset currentCrDs)
      throws IOException {
    // Setup scanner.
    CollectionLevelScanner scanner =
        new CollectionLevelScanner(collectionPath, collectionCrDs, catalogCrDs, currentCrDs, filter, service);
