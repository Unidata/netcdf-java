...
public class CrawlableDatasetDods implements CrawlableDataset {
  ...
  private static Map listDatasetsMap = null; // maintain an in-memory copy for performance reasons .. TODO: add a
                                             // version-check
  ...
  private String forceChild(String url) {
    ...
    int j = url.substring(0, url.length() - 1).lastIndexOf('/'); // url.length() - 1 was intentional .. if the last char
                                                                 // is a '/', we're interested in the previous one.
    if (j >= 0) {
  ...
  public List listDatasets() throws IOException {
    ...
    if (listDatasetsMap.containsKey(path)) // shortcut .. for performance
      ...
      List pathList = new ArrayList(); // only for detecting duplicates (after removing the extension, sometimes we end
                                       // up with duplicates)
      ...
      try {
        ...
        if (!openPath.endsWith("/")) // if you skip this, you will find that relative URLs don't work (fails in
                                     // "extract", and in particular in URL u = new URL(baseURL, value))
          openPath += "/";
        possibleDsList = urlExtractor.extract(openPath);
      } catch (java.io.IOException e) {
        log.warn("listDatasets(): IOException while extracting dataset info from given OPeNDAP directory <" + path
            + ">, return empty list: " + e.getMessage());
        return (list);
      }
      ...
      for (Iterator it = possibleDsList.iterator(); it.hasNext();) {
        ...
        if ((!isDodsDataset(curDsUrlString)) && (!isCollection(curDsUrlString))) {
          log.warn("expandThisLevel(): Dataset isn't an OPeNDAP dataset or collection dataset, skip <" + path + ">.");
          continue;
        }
        ...
        // Comment: this call was taken over from CrawlableDatasetFile. Since we use forceChild, this call is currently
        // useless.
        if (!curDsUrlString.startsWith(path)) {
          log.debug("listDatasets(): current path <" + curDsUrlString + "> not child of given" + " location <" + path
              + ">, skip.");
          continue;
        }
        ...
        try {
          ...
          log.error("listDatasets(): Skipping dataset  <" + curDsUrlString + "> due to URISyntaxException: "
              + e.getMessage());
          continue;
        }
        ...
        try {
          ...
          log.warn(
              "listDatasets(): Can't make CrawlableDataset for child url <" + curDsUrlString + ">: " + e.getMessage());
  ...
  public CrawlableDataset getParentDataset() {
    if (!path.equals("/")) {
      ...
      int index =
          parentPath.lastIndexOf("/", parentPath.endsWith("/") ? parentPath.length() - 2 : parentPath.length() - 1);
      if (index != -1)
