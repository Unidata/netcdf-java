 ...
 * processors. For example, when an application wants to create a new JAXP DocumentBuilderFactory instance, it calls the
 * staic method DocumentBuilderFactory.newInstance(). This causes a search for the name of a concrete subclass of
 * DocumentBuilderFactory using the following order:
 * 
 ...
 * The Jar Service Provider discovery mechanism specified in the Jar File Specification. A jar file can have a resource
 * (i.e. an embedded file) such as META-INF/services/javax.xml.parsers.DocumentBuilderFactory containing the name of the
 * concrete class to instantiate.
 * The fallback platform default implementation.
 * Of the above ways to specify an implementation, perhaps the most useful is the jar service provider mechanism. To use
 * this mechanism, place the implementation jar file on your classpath. For example, to use Xerces 1.4.4 instead of the
 * version of Crimson which is bundled with JDK 1.4 (Java Development Kit version 1.4), place xerces.jar in your
 * classpath. This mechanism also works with older versions of the JDK which do not bundle JAXP. If you are using JDK
 * 1.4 and above, see this question for potential problems.
 * 
 */
...
public class XMLEntityResolver implements org.xml.sax.EntityResolver {
  ...
  static {
     ...
     * System.out.
     * println("***Warning: XML Parser does not support JAXP 1.2. Library will continue without validation.\n"+e);
  ...
  public static void initEntity(String entityName, String resourceName, String urlName) {
    ...
    try (InputStream is = ucar.nc2.util.IO.getFileResource(resourceName)) { // try to read from local file resource, eg
      ...
      System.out.println(
          " *** FAILED to map entity " + entityName + " locally at " + resourceName + " or remotely at " + urlName);
      // e.printStackTrace();
    }
