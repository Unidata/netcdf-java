...
public class DIFWriter {
  ...
  private Iterator translateGribVocabulary(ThreddsMetadata.Variables vs, boolean isGrib1, StringBuilder mess) {
    ...
    for (ThreddsMetadata.Variable v : vs.getVariableList()) {
      ...
      if (hash.get(toVocabName) == null) {
        ThreddsMetadata.Variable transV =
            new ThreddsMetadata.Variable(v.getName(), v.getDescription(), toVocabName, v.getUnits(), fromVocabId);
  ...
  private void writeDataset(InvDataset ds, Element rootElem, StringBuilder mess) {
    ...
    if (tm != null) {
      ...
      tmElem.addContent(new Element("Stop_Date", defNS).addContent(tm.getEnd().toString()));
    }
    ...
    if (geo != null) {
      ...
      geoElem.addContent(new Element("Westernmost_Longitude", defNS).addContent(Double.toString(westNormal)));
      geoElem.addContent(new Element("Easternmost_Longitude", defNS).addContent(Double.toString(eastNormal)));
    }
  ...
  private void writeVariable(Element rootElem, ThreddsMetadata.Variable v) {
    ...
    int n = stoker.countTokens();
    if (n < 3)
      return; // gottta have the first 3
