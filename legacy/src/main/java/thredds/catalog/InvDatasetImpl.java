...
public class InvDatasetImpl extends InvDataset {
  ...
  static public void writeHtmlDescription(StringBuilder buff, InvDatasetImpl ds, boolean complete, boolean isServer,
    ...
    if ((ds.getDataFormatType() != null) && (ds.getDataFormatType() != DataFormatType.NONE))
      buff.append(" <li><em>Data format: </em>").append(StringUtil2.quoteHtmlContent(ds.getDataFormatType().toString()))
          .append("</li>\n");
    ...
    if ((ds.getDataType() != null) && (ds.getDataType() != FeatureType.ANY))
      buff.append(" <li><em>Data type: </em>").append(StringUtil2.quoteHtmlContent(ds.getDataType().toString()))
          .append("</li>\n");

    if ((ds.getCollectionType() != null) && (ds.getCollectionType() != CollectionType.NONE))
      buff.append(" <li><em>Collection type: </em>")
          .append(StringUtil2.quoteHtmlContent(ds.getCollectionType().toString())).append("</li>\n");

    if (ds.isHarvest())
      buff.append(" <li><em>Harvest: </em>").append(ds.isHarvest()).append("</li>\n");

    if (ds.getAuthority() != null)
      buff.append(" <li><em>Naming Authority: </em>").append(StringUtil2.quoteHtmlContent(ds.getAuthority()))
          .append("</li>\n");
    ...
    if (ds.getRestrictAccess() != null)
      buff.append(" <li><em>RestrictAccess: </em>").append(StringUtil2.quoteHtmlContent(ds.getRestrictAccess()))
          .append("</li>\n");
    ...
    if (docs.size() > 0) {
      ...
      for (InvDocumentation doc : docs) {
        String type =
            (doc.getType() == null) ? "" : "<strong>" + StringUtil2.quoteHtmlContent(doc.getType()) + ":</strong> ";
    ...
    if (contributors.size() > 0) {
      ...
      for (ThreddsMetadata.Contributor t : contributors) {
        String role =
            (t.getRole() == null) ? "" : "<strong> (" + StringUtil2.quoteHtmlContent(t.getRole()) + ")</strong> ";
    ...
    if (keywords.size() > 0) {
      ...
      for (ThreddsMetadata.Vocab t : keywords) {
        String vocab = (t.getVocabulary() == null) ? ""
            : " <strong>(" + StringUtil2.quoteHtmlContent(t.getVocabulary()) + ")</strong> ";
    ...
    if (dates.size() > 0) {
      ...
      for (DateType d : dates) {
        String type =
            (d.getType() == null) ? "" : " <strong>(" + StringUtil2.quoteHtmlContent(d.getType()) + ")</strong> ";
    ...
    if (creators.size() > 0) {
      ...
      for (ThreddsMetadata.Source t : creators) {
        ...
        if (t.getUrl() != null) {
          String newUrl = resolveRelativeUrls ? makeHrefResolve(ds, t.getUrl(), null) : makeHref(t.getUrl(), null);
    ...
    if (publishers.size() > 0) {
      ...
      for (ThreddsMetadata.Source t : publishers) {
        ...
        if (t.getUrl() != null) {
          String urlLink = resolveRelativeUrls ? makeHrefResolve(ds, t.getUrl(), null) : makeHref(t.getUrl(), null);
     ...
     * <ul>
     * <li><strong>Reflectivity</strong> = <i></i> = EARTH SCIENCE &gt; Spectral/Engineering &gt; Radar &gt; Radar
     * Reflectivity (db)
     * <li><strong>Velocity</strong> = <i></i> = EARTH SCIENCE &gt; Spectral/Engineering &gt; Radar &gt; Doppler
     * Velocity (m/s)
     * <li><strong>SpectrumWidth</strong> = <i></i> = EARTH SCIENCE &gt; Spectral/Engineering &gt; Radar &gt; Doppler
     * Spectrum Width (m/s)
     * </ul>
     * </ul>
     * </ul>
    ...
    if (vars.size() > 0) {
      ...
      for (ThreddsMetadata.Variables t : vars) {
        ...
        if (t.getVocabUri() != null) {
          ...
          String vocabLink = resolveRelativeUrls ? makeHrefResolve(ds, uri.toString(), t.getVocabulary())
        ...
        if (vlist.size() > 0) {
          for (ThreddsMetadata.Variable v : vlist) {
            ...
            buff.append(" <li><strong>").append(StringUtil2.quoteHtmlContent(v.getName() + units))
                .append("</strong> = ");
            String desc = (v.getDescription() == null) ? ""
                : " <i>" + StringUtil2.quoteHtmlContent(v.getDescription()) + "</i> = ";
            buff.append(desc);
    ...
    if ((gc != null) && !gc.isEmpty()) {
      ...
      if (gc.getUpDownRange() != null) {
        buff.append(" <li><em> Altitude: </em> ").append(rangeString(gc.getUpDownRange()))
            .append(" (positive is <strong>").append(StringUtil2.quoteHtmlContent(gc.getZPositive()))
            .append(")</strong></li>\n");
      }
    ...
    if (tc != null) {
      ...
      if (duration != null)
        buff.append(" <li><em>  Duration: </em> ").append(StringUtil2.quoteHtmlContent(duration.toString()))
            .append("\n");
      CalendarDuration resolution = tc.getResolution();
      if (resolution != null) {
        buff.append(" <li><em>  Resolution: </em> ").append(StringUtil2.quoteHtmlContent(resolution.toString()))
            .append("\n");
      }
      buff.append(" </ul>\n");
    }
    ...
    if (gotSomeMetadata) {
      ...
      for (InvMetadata m : metadata) {
        ...
        if (m.hasXlink()) {
          ...
          String mdLink =
              resolveRelativeUrls ? makeHrefResolve(ds, m.getXlinkHref(), title) : makeHref(m.getXlinkHref(), title);
    ...
    if (props.size() > 0) {
      ...
      for (InvProperty p : props) {
        if (p.getName().equals("attachments")) // LOOK whats this ?
        {
          ...
          buff.append(" <li>").append(StringUtil2.quoteHtmlContent(p.getName() + " = \"" + p.getValue()))
              .append("\"\n");
        }
      }
      buff.append("</ul>\n");
  ...
  boolean check(StringBuilder out, boolean show) {
    ...
    if (hasAccess() && (getDataType() == null)) {
      out.append("**Warning: Dataset (").append(getFullName())
          .append("): is selectable but no data type declared in it or in a parent element\n");
    }

    if (!hasAccess() && !hasNestedDatasets()) {
      out.append("**Warning: Dataset (").append(getFullName())
          .append("): is not selectable and does not have nested datasets\n");
    }

    if (show)
      System.out.println("  dataset " + name + " valid = " + isValid);
