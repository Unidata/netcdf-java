 ...
 *
 * <p>
 * Typically built from the information given by a datasetScan element in a
 * TDS config catalog.
 * </p>
 *
 * <p>
...
public class InvDatasetScan extends InvCatalogRef {
  ...
  public InvDatasetScan(InvDatasetImpl parent, String name, String path, String scanLocation, String id,
      InvDatasetScan from) {
    this(parent, name, path, scanLocation, from.crDsClassName, from.crDsConfigObj, from.filter, from.identifier,
        from.namer, from.addDatasetSize, from.sorter, from.proxyDatasetHandlers, from.childEnhancerList,
   ...
   */
  public InvDatasetScan(InvCatalogImpl catalog, InvDatasetImpl parent, String name, String path, String scanLocation,
      String filter, boolean addDatasetSize, String addLatest, boolean sortOrderIncreasing,
      String datasetNameMatchPattern, String startTimeSubstitutionPattern, String duration) {

    super(parent, name, makeHref(path));
    log.debug("InvDatasetScan(): parent=" + parent + ", name=" + name + " , path=" + path + " , scanLocation="
        + scanLocation + " , filter=" + filter + " , addLatest=" + addLatest + " , sortOrderIncreasing="
        + sortOrderIncreasing + " , datasetNameMatchPattern=" + datasetNameMatchPattern
        + " , startTimeSubstitutionPattern= " + startTimeSubstitutionPattern + ", duration=" + duration);
    ...
    if (this.scanLocationCrDs == null) {
      isValid = false;
      invalidMessage = new StringBuilder("Invalid InvDatasetScan <path=").append(path).append("; scanLocation=")
          .append(scanLocation).append(">: could not create CrawlableDataset for scanLocation.");
    } else if (!this.scanLocationCrDs.exists()) {
      isValid = false;
      invalidMessage = new StringBuilder("Invalid InvDatasetScan <path=").append(path).append("; scanLocation=")
          .append(scanLocation).append(">: CrawlableDataset for scanLocation does not exist.");
    } else if (!this.scanLocationCrDs.isCollection()) {
      isValid = false;
      invalidMessage = new StringBuilder("Invalid InvDatasetScan <path=").append(path).append("; scanLocation=")
          .append(scanLocation).append(">: CrawlableDataset for scanLocation not a collection.");
    }
    ...
    if (datasetNameMatchPattern != null && startTimeSubstitutionPattern != null && duration != null) {
      ...
      childEnhancerList.add(RegExpAndDurationTimeCoverageEnhancer
          .getInstanceToMatchOnDatasetName(datasetNameMatchPattern, startTimeSubstitutionPattern, duration));
    }
   ...
   */
  public InvDatasetScan(InvCatalogImpl catalog, InvDatasetImpl parent, String name, String path, String scanLocation,
      CrawlableDatasetFilter filter, boolean addDatasetSize, String addLatest, boolean sortOrderIncreasing,
      String datasetNameMatchPattern, String startTimeSubstitutionPattern, String duration) {
  ...
  public InvDatasetScan(InvDatasetImpl parent, String name, String path, String scanLocation, String configClassName,
      Object configObj, CrawlableDatasetFilter filter, CrawlableDatasetLabeler identifier,
      CrawlableDatasetLabeler namer, boolean addDatasetSize, CrawlableDatasetSorter sorter,
      Map<String, ProxyDatasetHandler> proxyDatasetHandlers, List<DatasetEnhancer> childEnhancerList,
  ...
  public void setScanLocation(String scanLocation) {
    ...
    if (!scanLocation.equals(this.scanLocation)) {
      ...
      if (this.scanLocationCrDs == null) {
        isValid = false;
        invalidMessage = new StringBuilder("Invalid InvDatasetScan <path=").append(rootPath).append("; scanLocation=")
            .append(scanLocation).append(">: could not create CrawlableDataset for scanLocation.");
      } else if (!this.scanLocationCrDs.exists()) {
        isValid = false;
        invalidMessage = new StringBuilder("Invalid InvDatasetScan <path=").append(rootPath).append("; scanLocation=")
  ...
  public String getCrDsClassName() {
    return crDsClassName;
  }

  public Object getCrDsConfigObj() {
    return crDsConfigObj;
  }
  ...
  public List<DatasetEnhancer> getChildEnhancerList() {
    return childEnhancerList;
  }

  public CatalogRefExpander getCatalogRefExpander() {
    return catalogRefExpander;
  }
  ...
  private CatalogBuilder buildCatalogBuilder() {
    ...
    try {
      ...
      log.error("buildCatalogBuilder(): failed to create CatalogBuilder for this collection <"
          + scanLocationCrDs.getPath() + ">: " + e.getMessage());
      return null;
    }
   ...
   * @throws IllegalArgumentException if the given path is not a descendant of (or the same as) this InvDatasetScan
   *         collection level.
   */
  ...
  public InvCatalogImpl makeCatalogForDirectory(String orgPath, URI catURI) {
    ...
    if (dsDirPath == null) {
      String tmpMsg =
          "makeCatalogForDirectory(): Requesting path <" + orgPath + "> must start with \"" + rootPath + "\".";
      log.error(tmpMsg);
      return null;
    }
    ...
    try {
      ...
      log.error(
          "makeCatalogForDirectory(): catalog generation failed <" + catalogCrDs.getPath() + ">: " + e.getMessage());
      return null;
    }
  ...
  public InvCatalogImpl makeProxyDsResolverCatalog(String path, URI baseURI) {
    ...
    try {
      ...
      log.error("makeProxyDsResolverCatalog(): failed to create CrawlableDataset for catalogLevel <" + dsDirPath
          + "> and class <" + crDsClassName + ">: " + e.getMessage(), e);
      return null;
    }
    if (catalogCrDs == null) {
    ...
    try {
      ...
      log.error(
          "makeProxyDsResolverCatalog(): catalog generation failed <" + catalogCrDs.getPath() + ">: " + e.getMessage());
      return null;
    }
   ...
   * @return the catalog for this path (uses version 1.1) or null if build unsuccessful.
   *
   * @deprecated Instead use {@link #makeProxyDsResolverCatalog(String, URI) makeProxyDsResolver()} which provides more
   *             general proxy dataset handling.
   */
  public InvCatalog makeLatestCatalogForDirectory(String orgPath, URI baseURI) {
    ...
    InvDatasetImpl latestDs =
        (InvDatasetImpl) java.util.Collections.max(topDs.getDatasets(), new java.util.Comparator() {
