...
public class DeepCopyUtils {
  ...
  public static InvCatalog copyCatalog(InvCatalog catalog) {
    ...
    InvCatalogImpl resultCatalog =
        new InvCatalogImpl(catalog.getName(), "1.0", catalog.getExpires(), ((InvCatalogImpl) catalog).getBaseURI());
  ...
  public static InvCatalog subsetCatalogOnDataset(InvCatalog catalog, String datasetId) {
    ...
    if (ds == null)
      throw new IllegalArgumentException(
          "The dataset ID [" + datasetId + "] does not match the ID of a dataset in the catalog.");
  ...
  private static URI formDocBaseUriForSubsetCatalog(InvCatalog catalog, InvDataset dataset) {
    ...
    try {
      ...
      // String uriPath = thisDocBaseUri.getPath() + "/" + ( dataset.getID() != null ? dataset.getID() :
      // dataset.getName() );
      // URI subsetDocBaseUri = new URI( thisDocBaseUri.getScheme(), thisDocBaseUri.getUserInfo(),
      // thisDocBaseUri.getHost(),
      // thisDocBaseUri.getPort(), uriPath, null, null );
      return new URI(subsetDocBaseUriString);
    } catch (URISyntaxException e) {
      // This shouldn't happen. But just in case ...
      throw new IllegalStateException("Bad document Base URI for new catalog [" + catalog.getUriString() + "/"
          + (dataset.getID() != null ? dataset.getID() : dataset.getName()) + "].", e);
    }
  }

  // public static InvDataset copyDatasetAddingInheritedMetadata( InvDataset dataset, List<ThreddsMetadata> inheritedMd,
  ...
  public static InvService copyService(InvService service) {
    ...
    InvService resultService = new InvService(service.getName(), service.getServiceType().toString(), service.getBase(),
