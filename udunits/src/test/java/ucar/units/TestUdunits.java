...
public class TestUdunits {
  ...
  public void testOffsetUnit()
    ...
    final BaseUnit kelvin = BaseUnit.getOrCreate(UnitName.newUnitName("kelvin", null, "K"),
    ...
    System.out.println("celsius.toDerivedUnit(new float[]{1,2,3}, new float[3])[1]=" + celsius
        .toDerivedUnit(new float[]{1, 2, 3}, new float[3])[1]);
    System.out.println("celsius.fromDerivedUnit(274.15)=" + celsius.fromDerivedUnit(274.15));
    System.out.println("celsius.fromDerivedUnit(new float[]{274.15f},new float[1])[0]=" + celsius
        .fromDerivedUnit(new float[]{274.15f}, new float[1])[0]);
    ...
    final BaseUnit radian = BaseUnit
        .getOrCreate(UnitName.newUnitName("radian", null, "rad"), BaseQuantity.PLANE_ANGLE);
  ...
  public void testLogarithmicUnit()
      throws IOException, NameException, UnitExistsException, OperationException, ConversionException {
    final BaseUnit meter = BaseUnit
        .getOrCreate(UnitName.newUnitName("meter", null, "m"), BaseQuantity.LENGTH);
  ...
  public void testBaseQuantity() {
    System.out.println("AMOUNT_OF_SUBSTANCE.getName() = "
        + BaseQuantity.AMOUNT_OF_SUBSTANCE.getName());
    System.out.println("LUMINOUS_INTENSITY.getSymbol() = "
        + BaseQuantity.LUMINOUS_INTENSITY.getSymbol());
    System.out.println("PLANE_ANGLE.getSymbol() = "
        + BaseQuantity.PLANE_ANGLE.getSymbol());

    System.out
        .println("LENGTH.equals(LENGTH) = " + BaseQuantity.LENGTH.equals(BaseQuantity.LENGTH));
    ...
    System.out.println("LENGTH.equals(PLANE_ANGLE) = "
        + BaseQuantity.LENGTH.equals(BaseQuantity.PLANE_ANGLE));
    System.out.println("PLANE_ANGLE.equals(PLANE_ANGLE) = "
        + BaseQuantity.PLANE_ANGLE.equals(BaseQuantity.PLANE_ANGLE));
    System.out.println("PLANE_ANGLE.equals(SOLID_ANGLE) = "
        + BaseQuantity.PLANE_ANGLE.equals(BaseQuantity.SOLID_ANGLE));

    System.out.println("LENGTH.compareTo(LENGTH) = "
        + BaseQuantity.LENGTH.compareTo(BaseQuantity.LENGTH));
    System.out
        .println("LENGTH.compareTo(MASS) = " + BaseQuantity.LENGTH.compareTo(BaseQuantity.MASS));
    System.out.println("LENGTH.compareTo(PLANE_ANGLE) = "
        + BaseQuantity.LENGTH.compareTo(BaseQuantity.PLANE_ANGLE));
    System.out.println("PLANE_ANGLE.compareTo(PLANE_ANGLE) = "
        + BaseQuantity.PLANE_ANGLE.compareTo(BaseQuantity.PLANE_ANGLE));
    System.out.println("PLANE_ANGLE.compareTo(SOLID_ANGLE) = "
        + BaseQuantity.PLANE_ANGLE.compareTo(BaseQuantity.SOLID_ANGLE));
  }
  ...
  private static void myAssert(StandardUnitFormat parser, final String spec, final Unit unit)
      throws Exception {
