...
public class GridHorizCoordSys {
  ...
  public GridHorizCoordSys(GridDefRecord gds, GridTableLookup lookup, Group g) {
    ...
    id = (g == null) ? grid_name : g.getFullName();
  ...
  void addToNetcdfFile(NetcdfFile ncfile) {
    ...
    if (isLatLon) {
      ...
      if (gds.getDouble(GridDefRecord.DY) == GridDefRecord.UNDEFINED) {
        ...
        dy = (gds.getDouble(GridDefRecord.LA2) < gds.getDouble(GridDefRecord.LA1)) ? -gds.getDouble(GridDefRecord.DY)
            : gds.getDouble(GridDefRecord.DY);
      }
      ...
      if (isGaussian) {
        ...
        addCoordAxis(ncfile, "lat", gds.getInt(GridDefRecord.NY), gds.getDouble(GridDefRecord.LA1), dy, "degrees_north",
            "latitude coordinate", "latitude", AxisType.Lat);
      }

      // lon
      addCoordAxis(ncfile, "lon", gds.getInt(GridDefRecord.NX), gds.getDouble(GridDefRecord.LO1),
          gds.getDouble(GridDefRecord.DX), "degrees_east", "longitude coordinate", "longitude", AxisType.Lon);
      ...
      if (makeProjection(ncfile, projType)) {
        ...
        if (projType == GridTableLookup.RotatedLatLon) {
          double dy = (gds.getDouble("La2") < gds.getDouble(GridDefRecord.LA1) ? -gds.getDouble(GridDefRecord.DY)
              : gds.getDouble(GridDefRecord.DY));

          yData = addCoordAxis(ncfile, "y", gds.getInt(GridDefRecord.NY), gds.getDouble(GridDefRecord.LA1), dy,
              "degrees", "y coordinate of projection", "projection_y_coordinate", AxisType.GeoY);
          xData = addCoordAxis(ncfile, "x", gds.getInt(GridDefRecord.NX), gds.getDouble(GridDefRecord.LO1),
              gds.getDouble(GridDefRecord.DX), "degrees", "x coordinate of projection", "projection_x_coordinate",
              AxisType.GeoX);
          ...
          yData = addCoordAxis(ncfile, "y", gds.getInt(GridDefRecord.NY), starty, getDyInKm(), "km",
              "y coordinate of projection", "projection_y_coordinate", AxisType.GeoY);
          xData = addCoordAxis(ncfile, "x", gds.getInt(GridDefRecord.NX), startx, getDxInKm(), "km",
              "x coordinate of projection", "projection_x_coordinate", AxisType.GeoX);
        }
   ...
   * @return the values
   */
  private double[] addGaussianLatAxis(NetcdfFile ncfile, String name, String units, String desc, String standard_name,
      AxisType axis) {
    ...
    if (Double.isNaN(np))
      throw new IllegalArgumentException(
          "Gaussian Lat/Lon grid must have 'NumberParallels' or 'Np' (number of parallels) parameter");
  ...
  private boolean makeProjection(NetcdfFile ncfile, int projType) {
    switch (projType) {
      ...
      default:
        throw new UnsupportedOperationException("unknown projection = " + gds.getInt(GridDefRecord.GRID_TYPE));
    }
  ...
  private void makeLC() {
    ...
    if (GridServiceProvider.debugProj) {
      ...
      ProjectionPoint endPP = proj.latLonToProj(endLL);
  ...
  private void makePS() {
    ...
    ProjectionPoint start =
        proj.latLonToProj(LatLonPoint.create(gds.getDouble(GridDefRecord.LA1), gds.getDouble(GridDefRecord.LO1)));
    startx = start.getX();
    starty = start.getY();
  ...
  private void makeRotatedLatLon(NetcdfFile ncfile) {
    ...
    if (GridServiceProvider.debugProj) {
      ...
      System.out.println("Lon=" + gds.getDouble(GridDefRecord.LO1) + ", " + "Lat=" + gds.getDouble(GridDefRecord.LA1));
      ...
      double dy =
          (La2 < gds.getDouble(GridDefRecord.LA1)) ? -gds.getDouble(GridDefRecord.DY) : gds.getDouble(GridDefRecord.DY);
  ...
  private void makeSpaceViewOrOthographic() {
    ...
    double apparentDiameter = 2 * Math.sqrt((nr - 1) / (nr + 1)); // apparent diameter, units of radius (see Snyder p
                                                                  // 173)
   ...
   * 65-68 Orientation of the grid; i.e., the angle between the increasing Y-axis and the meridian of the sub-satellite
   * point in the direction of increasing latitude (see Note 3)
   * 69-72 Nr - altitude of the camera from the Earths centre, measured in units of the Earth (equatorial) radius
   * multiplied by a scale factor of 10 6 (see Notes 4 and 5)
   ...
   * This is correctly encoded in our products.
   * 
   * Due to the elliptical shape of the earth, the calculation is a bit different in the y direction (Nr is in multiples
   * of
  ...
  private void makeMSGgeostationary() {
    ...
    if (Double.isNaN(minor_axis))
      minor_axis = gds.getDouble("minor_axis_earth");
    // Nr = altitude of camera from center, in units of radius
    double nr = gds.getDouble(GridDefRecord.NR) * 1e-6; // altitude of the camera from the Earths centre, measured in
                                                        // units of the Earth (equatorial) radius
  ...
  private void setDxDy(double startx, double starty, Projection proj) {
    ...
    LatLonPoint endLL = LatLonPoint.create(La2, Lo2);
    ProjectionPoint end = proj.latLonToProj(endLL);
