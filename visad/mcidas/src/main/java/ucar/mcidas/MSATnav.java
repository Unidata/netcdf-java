...
public final class MSATnav extends AREAnav {
  ...
  public double[][] toLatLon(double[][] linele) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      if ((val1 * val2) > ((yk * yk) / (yk * yk - 1))) {
        ...
        vmu = (rs - (EARTH_RADIUS * (Math.sqrt((yk * yk) - (yk * yk - 1) * val1 * val2)))) / (val1 * val2);
        ...
        xfi = (Math.atan(((Math.tan(teta)) * EARTH_RADIUS) / rp)) * RADIANS_TO_DEGREES;
  ...
  public double[][] toLinEle(double[][] latlon) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      y1 = isEastPositive ? latlon[indexLon][point] : -latlon[indexLon][point];
