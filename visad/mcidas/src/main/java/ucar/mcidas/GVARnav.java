...
public class GVARnav extends AREAnav {
  ...
  final int RELLST[][] = {{4, 10}, {13, 63}, {65, 94}, {98, 100}, {103, 105}, {108, 110}, {113, 115}, {116, 118},
      {120, 149}, {153, 155}, {158, 160}, {163, 165}, {168, 170}, {171, 173}, {175, 204}, {208, 210}, {213, 215},
      {218, 220}, {223, 225}, {226, 228}, {230, 259}, {263, 265}, {268, 270}, {273, 275}, {278, 283}, {285, 314},
      {318, 320}, {323, 325}, {328, 330}, {333, 335}, {-1, -1}};
  ...
  public GVARnav(int ifunc, int[] iparms) throws IllegalArgumentException {
    ...
    if (iparms[STTYPE] != GVAR)
      throw new IllegalArgumentException("Invalid navigation type" + iparms[STTYPE]);
    ...
    while (RELLST[count][0] != -1) {
      ...
      offset = 1;
      if (RELLST[count][0] > CUTOF1)
        offset = 13;
      if (RELLST[count][0] > CUTOF2)
        offset = 31;

      for (loop = RELLST[count][0]; loop <= RELLST[count][1]; loop++) {
        if ((loop == 13) || (loop == 60) || (((loop - 7) % 55) == 0) && (loop != 7)) {
    ...
    int imc = 1;
    if ((iparms[IMCACT] & (1 << IMCFLG)) != 0)
      imc = 0;
    iflip = 1;
    if ((iparms[IYFLIP] & (1 << FLPFLG)) != 0)
      iflip = -1;
    ...
    if (imc != 0) {
      ...
      lam = lam + rparms[LDR1] + (rparms[LDR2] + rparms[LDR3] * w) * w
          + (rparms[LDR10] * sw1 + rparms[LDR11] * cw1 + rparms[LDR4] * sw + rparms[LDR5] * cw + rparms[LDR6] * s2w
              + rparms[LDR7] * c2w + rparms[LDR8] * sw3 + rparms[LDR9] * cw3
              + w * (rparms[LDR12] * sw + rparms[LDR13] * cw)) * 2.;

      // computes change in radial distance from the reference (km)
      dr = dr + rparms[RDDR1] + rparms[RDDR2] * cw + rparms[RDDR3] * sw + rparms[RDDR4] * c2w + rparms[RDDR5] * s2w
          + rparms[RDDR6] * cw3 + rparms[RDDR7] * sw3 + rparms[RDDR8] * cw1 + rparms[RDDR9] * sw1
          + w * (rparms[RDDR10] * cw + rparms[RDDR11] * sw);

      // computes the sine of the change in the geocentric latitude
      dlat = rparms[DGL1] + rparms[DGL2] * cw + rparms[DGL3] * sw + rparms[DGL4] * c2w + rparms[DGL5] * s2w
          + w * (rparms[DGL6] * cw + rparms[DGL7] * sw) + rparms[DGL8] * cw1 + rparms[DGL9] * sw1;
      ...
      dyaw = rparms[DOY1] + rparms[DOY2] * sw + rparms[DOY3] * cw + rparms[DOY4] * s2w + rparms[DOY5] * c2w
          + w * (rparms[DOY6] * sw + rparms[DOY7] * cw) + rparms[DOY8] * sw1 + rparms[DOY9] * cw1;
  ...
  private double gatt(int k, float rparms[], int iparms[], double wa, double te) {
    ...
    for (int loop = 1; loop <= i; loop++) {
      att = att + rparms[k + 2 * loop + 2] * Math.cos(wa * (double) loop + rparms[k + 2 * loop + 3]);
    }
    ...
    for (int l = 1; l <= kkk; l++) {
      ...
      att = att + rparms[ll - 2] * Math.pow((wa - rparms[ll]), mr) * Math.cos(jr * wa + rparms[ll - 1]);
    }
  ...
  public double[][] toLinEle(double[][] latlon) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      if (w2 <= 0.0) {
        ...
        w1 = Math.sin(alf);
        w2 = Math.cos(gam);
        alpha1 = alf + rma * (1. - Math.cos(alf) / w2) + pma * w1 * (doff / w2 + Math.tan(gam));
        gam = gam - ff * rma * w1;
