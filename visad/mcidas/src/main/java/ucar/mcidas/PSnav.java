...
public final class PSnav extends AREAnav {
  ...
  public PSnav(int[] iparms) throws IllegalArgumentException {
    ...
    xlat1 = McIDASUtil.integerLatLonToDouble(ipole - iparms[3]) * DEGREES_TO_RADIANS;
  ...
  public double[][] toLatLon(double[][] linele) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      if (Math.abs(radius) < 1.e-10)
        ...
        xlat = ihem * (90. - 2 * Math.atan(Math.exp(Math.log(radius / fac))) / DEGREES_TO_RADIANS);
  ...
  public double[][] toLinEle(double[][] latlon) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      xlon = (iwest == 1) ? -latlon[indexLon][point] : latlon[indexLon][point];
