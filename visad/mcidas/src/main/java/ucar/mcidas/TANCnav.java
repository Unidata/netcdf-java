...
public final class TANCnav extends AREAnav {
  ...
  public TANCnav(int[] iparms) throws IllegalArgumentException {
    ...
    lat0 = iparms[4] / 10000.;
    lon0 = iparms[5] / 10000.;
    if (scale <= 0 || lat0 <= -90. || lat0 >= 90. || lat0 == 0 || lon0 <= -180. || lon0 >= 180.)
  ...
  public double[][] toLatLon(double[][] linele) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      double theta;
      if (lat0 < 0.) {
        theta = (theta_rh <= 0.) ? Math.PI - Math.abs(theta_rh) : -1. * (Math.PI - Math.abs(theta_rh));
      } else
        theta = theta_rh;
      ...
      if (theta <= -mxtheta || theta > mxtheta) {
        ...
        if (lon > Math.PI)
          lon = lon - 2. * Math.PI;
        double colat = 2. * Math.atan(tancl2 * Math.pow(radius / (Erad * tancl), 1. / coscl));
  ...
  public double[][] toLinEle(double[][] latlon) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      if (lat <= -90. || lat >= 90. || lon <= -360. || lon > 360.) {
        ...
        double colat = (lat0 < 0) ? Math.PI / 2. + DEGREES_TO_RADIANS * lat : Math.PI / 2. - DEGREES_TO_RADIANS * lat;
        double in_lon = DEGREES_TO_RADIANS * lon;
        // map longitude into range -Pi to Pi
        if (in_lon <= -Math.PI)
          in_lon = in_lon + 2. * Math.PI;
        if (in_lon > Math.PI)
          in_lon = in_lon - 2. * Math.PI;
        ...
        if (colat == Math.PI) {
          ...
          double radius = Erad * tancl * Math.pow(Math.tan(colat / 2.) / tancl2, coscl);
          ...
          if (lat0 < 0)
            theta = Math.PI + theta;
          linele[indexLine][point] = lin0 + radius * Math.cos(theta) / scale;
          linele[indexEle][point] = ele0 + radius * Math.sin(theta) / scale;
        }
      }
    } // end point loop
