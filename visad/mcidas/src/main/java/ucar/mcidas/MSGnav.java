...
public final class MSGnav extends AREAnav {
  ...
  public double[][] toLatLon(double[][] linele) {
    ...
    for (int point = 0; point < number; point++) {
      if (Double.isNaN(imglinele[indexLine][point]) || Double.isNaN(imglinele[indexEle][point])) {
        continue;
      }
  ...
  public double[][] toLinEle(double[][] latlon) {
    ...
    for (int point = 0; point < number; point++) {
      if (Double.isNaN(latlon[indexLat][point]) || Double.isNaN(latlon[indexLon][point])) {
      ...
      xfi = xlat * cdr;
      xla = xlon * cdr;
      rom = EARTH_RADIUS * rp / Math
          .sqrt(rp * rp * Math.cos(xfi) * Math.cos(xfi) + EARTH_RADIUS * EARTH_RADIUS * Math.sin(xfi) * Math.sin(xfi));
  ...
  public float[][] toLatLon(float[][] linele) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      if (Float.isNaN(imglinele[indexLine][point]) || Float.isNaN(imglinele[indexEle][point])) {
        continue;
      }
      xlin = 3713. - imglinele[indexLine][point] / 3.0;
      xele = 3713. - imglinele[indexEle][point] / 3.0;
  ...
  public float[][] toLinEle(float[][] latlon) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      if (Float.isNaN(latlon[indexLat][point]) || Float.isNaN(latlon[indexLon][point])) {
