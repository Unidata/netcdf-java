...
public final class GOESnav extends AREAnav {
  ...
  public GOESnav(int[] iarr) throws IllegalArgumentException {
    ...
    if (iarr[28] == McIDASUtil.MCMISSING)
      skew = 0.;
    ...
    double raha = McIDASUtil.timdif(74001, 0, navday, 0) * 1.00273791 / 4.0 + 100.26467;
  ...
  public float[][] toLatLon(float[][] linele) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      cq = (Math.pow(xyz[0], 2) + Math.pow(xyz[1], 2)) * basq + Math.pow(xyz[2], 2) - bsq;
  ...
  public float[][] toLinEle(float[][] latlon) {
    ...
    for (int point = 0; point < number; point++) {
      ...
      ypar = isEastPositive ? -latlon[indexLon][point] : latlon[indexLon][point];
      ...
      if (Math.abs(xpar) <= 90.) {
        ...
        for (int i = 0; i < 2; i++) {
          ...
          znorm = Math.sqrt(Math.pow(vcste1, 2) + Math.pow(vcste2, 2) + Math.pow(vcste3, 2));
          x3 = vcses3 / znorm;
          umv = Math.atan2(x3, Math.sqrt(rfact - Math.pow(x3, 2))) - roasin;
        ...
        if (cosa >= ctst) {
          ...
          xele = picele + Math.asin((xsats1 * vcses2 - ysats2 * vcses1) / (xnorm * ynorm)) / radele;
  ...
  private double[] satvec(double samtim) {
    if (iold != 1) {
      ...
      double te = 1440.0 * de + 60.0 * McIDASUtil.mcPackedIntegerToDouble(ietimh);
      int iray = irayd / 1000;
      int irad = irayd % 1000;
