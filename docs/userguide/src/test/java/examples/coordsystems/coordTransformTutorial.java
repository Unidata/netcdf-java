...
public class coordTransformTutorial {
  ...
  public static void projectionEx() {
    /* INSERT public */class LambertConformalConic extends AbstractProjectionCT
        implements HorizTransformBuilderIF {

      public ProjectionCT.Builder<?> makeCoordinateTransform(AttributeContainer ctv,
          @Nullable String geoCoordinateUnits) {
        return null;
      }
  ...
  public static ProjectionCT.Builder<?> implementMakeCoordinateTransform(AttributeContainer ctv,
    ...
    double par2 =
        (parAtt.getLength() > 1) ? Objects.requireNonNull(parAtt.getNumericValue(1)).doubleValue()
            : par1;
    ...
    if (!Double.isNaN(semi_major_axis)
        && (!Double.isNaN(semi_minor_axis) || inverse_flattening != 0.0)) {
      Earth earth = new Earth(semi_major_axis, semi_minor_axis, inverse_flattening);
      proj = new ucar.unidata.geoloc.projection.proj4.LambertConformalConicEllipse(lat0, lon0, par1,
          par2, false_easting, false_northing, earth);

    } else {
      // A Projection is created from the parameters
      proj = new ucar.unidata.geoloc.projection.LambertConformal(lat0, lon0, par1, par2,
          false_easting, false_northing, EARTH_RADIUS);
    }
  ...
  public static void vertTransEx() {
    /* INSERT public */class CFOceanSigma extends AbstractVerticalCTBuilder
  ...
  public static VerticalCT.Builder<?> implementMakeVerticalCT(NetcdfFile ds, AttributeContainer ctv,
      String sigma, String ps, String ptop) {
    /*
    ...
    rs.toBuilder().addParameter(
        new Attribute("formula", "pressure(x,y,z) = ptop + sigma(z)*(surfacePressure(x,y)-ptop)"));
  ...
  public static void vertTransClass() {
    ...
    class AtmosSigma {/* DOCS-IGNORE */
      ...
      public ucar.unidata.geoloc.vertical.AtmosSigma create(NetcdfFile ds, Dimension timeDim,
          List<Attribute> params) {
        // IMPLEMENTED BELOW
        return null; /* DOCS-IGNORE */
      }

      public AtmosSigma(Dimension timeDim, String units, Variable psVar, double[] sigma,
          double ptop) {
        // IMPLEMENTED BELOW
      }
