...
* [nccompare](#nccompare): compares two [CDM files](../developer/file_types.html){:target="_blank"} for semantic equivalence
...
This application works on any [CDM file](../developer/file_types.html){:target="_blank"}, not just netCDF files.

~~~bash
java -Xmx1g -classpath netcdfAll-<version>.jar ucar.nc2.write.NcdumpArray
...
Compares two [CDM files](../developer/file_types.html){:target="_blank"} for semantic equivalence.
...
Write GRIB Collection Indexes from an XML file containing a [GRIB `<featureCollection>`](../developer/grib_feature_collections_ref.html){:target="_blank"} XML element.
...
Scans all the files in a directory to see if they are [CDM files](../developer/file_types.html){:target="_blank"} and can be identified as a particular feature type.
