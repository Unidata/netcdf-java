...
public class TestRadialDataset {
  ...
  private void testAllMethods(String location) throws IOException {
    ...
    RadialDatasetSweep rds =
        (RadialDatasetSweep) FeatureDatasetFactoryManager.open(FeatureType.RADIAL, location, null, errlog);
  ...
  private void testSweep(RadialDatasetSweep.Sweep s) throws IOException {
    ...
    if (show)
      System.out.println("  sweep type= " + s.getType() + " nRadials= " + s.getRadialNumber() + " nGates= "
          + s.getGateNumber() + " width= " + s.getBeamWidth() + " nyqFreq= " + s.getNyquistFrequency() + " firstGate= "
          + s.getRangeToFirstGate() + " sizeGate= " + s.getGateSize() + " meanElev= " + s.getMeanElevation()
          + " meanAzi= " + s.getMeanAzimuth() + " startingTime= " + s.getStartingTime() + " endingTime= "
          + s.getEndingTime());
    long start = System.currentTimeMillis();
