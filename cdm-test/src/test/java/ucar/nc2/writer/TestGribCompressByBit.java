...
public class TestGribCompressByBit {
  ...
  enum Action {
    floats, floatShaved, rawInts
  }
  enum ExtraAction {
    entropyB, entropyI
  }
  enum Algorithm {
    deflate, bzip2, bzip2T, bzipScaled, zip7
  }
  ...
  TestGribCompressByBit(Action act, ExtraAction[] aa, Algorithm... wantAlgs) {
    ...
    for (Algorithm want : wantAlgs) {
      switch (want) {
        case deflate:
          runAlg.add(new JavaDeflate());
          break;
        case bzip2:
          runAlg.add(new ApacheBzip2());
          break;
        // case bzip2T: runAlg.add( new ItadakiBzip2()); break;
        case bzipScaled:
          runAlg.add(new ScaleAndOffset());
          break;
  ...
  private int doGrib2(boolean showDetail, String filename) {
    ...
    try (RandomAccessFile raf = new RandomAccessFile(filename, "r")) {
      ...
      while (reader.hasNext()) {
        ...
        if (template != 40)
          continue; // skip all but JPEG-2000
        ...
        // (int nbits, int dataPoints, int gribMsgLength, long gribReadTime, Bean bean, byte[] bdata, int[] rawData,
        // Grib2Record gr) throws IOException {

        makeDetailReport(nBits, info.ndataPoints, info.bitmapLength, info.msgLength, gribReadTime,
            new Bean(info, fdata), bdata, rawData, gr);
  ...
  private int doGrib1(boolean showDetail, String filename) {
    ...
    try (RandomAccessFile raf = new RandomAccessFile(filename, "r")) {
      ...
      while (reader.hasNext()) {
        ...
        makeDetailReport(nBits, info.ndataPoints, info.bitmapLength, info.msgLength, gribReadTime,
            new Bean(info, fdata), bdata, rawData, null);
  ...
  private void makeDetailHeader(boolean showDetail, String filename, Action action, int npoints)
    ...
    // results
    if (detailOut != null)
      detailOut.printf("%s", f.toString());
    System.out.printf("%s", f.toString());
  }

  private void makeDetailReport(int nbits, int dataPoints, int bitmapLength, long gribMsgLength, long gribReadTime,
      Bean bean, byte[] bdata, int[] rawData, Grib2Record gr2) throws IOException {
    Formatter f = new Formatter();

    double entropyI = doEntropyI && rawData != null ? GribData.entropy(nbits, rawData) : 0.0;
    if (gr2 != null && Double.isNaN(entropyI))
      showData(gr2, bean, rawData); // debug
    ...
    for (CompressAlgo alg : runAlg) {
      ...
      f.format("%d, %f, %d, %d, ", alg.compressSize, ratio, alg.nsecsCompress / 1000 / 1000,
          alg.nsecsUncompress / 1000 / 1000);
    }
    f.format("%n");
   ...
   * 
   * // results
   * if (detailOut != null) detailOut.printf("%d, %d, %d, %d, %d%n", info.numbits, data.length, info.msgLength,
   * compressDeflate, compressOrg);
   * //System.out.printf("%d, %d, %d, %d, %f, %d%n", info.numbits, data.length, info.msgLength, compressBytes, r,
   * compressOrg);
   * nrecords++;
  ...
  public static void main(String[] args) throws IOException {
    ...
    try {
      ...
      TestGribCompressByBit compressByBit =
          new TestGribCompressByBit(Action.rawInts, extras, Algorithm.deflate, Algorithm.bzip2T, Algorithm.zip7);
  ...
  private static class MyFileFilter implements FileFilter {
    ...
    public boolean accept(File pathname) {
      ...
      if (name.contains("GEFS_Global_1p0deg_Ensemble"))
   ...
   * try {
   * String dirName = "Q:/cdmUnitTest/tds/ncep/";
   * TestDir.actOnAll(dirName, new TestDir.FileFilterFromSuffixes("grib2"), new CompressReportAction(true, false),
   * false);
   * 
   * } finally {
   * summaryOut.close();
