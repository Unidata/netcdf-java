...
public class OpendapServlet extends AbstractServlet implements InitializingBean {
  ...
  public void doGetASC(ReqState rs) throws Exception {
    ...
    try (GuardedDataset ds = getDataset(rs)) {
  ...
  private void checkSize(ServerDDS dds, boolean isAscii) throws Exception {
    ...
    double maxSize = isAscii ? opendapAscLimit : opendapBinLimit; // Mbytes
    if (dsize > maxSize) {
  ...
  long computeFieldSize(BaseType bt, boolean isAscii) throws Exception {
    ...
    if (bt instanceof DConstructor) {
      ...
      if (dp instanceof DString) {
        ...
        DataType dtype = DodsNetcdfFiles.convertToNCType(bt, false);
  ...
  long computeArraySize(SDArray da) throws Exception {
    ...
    DataType dtype = DodsNetcdfFiles.convertToNCType(base, false);
  ...
  protected GuardedDataset getDataset(ReqState preq) throws Exception {
    ...
    if (acceptSession) {
      ...
      // LOOK session.setAttribute(CookieFilter.SESSION_PATH, cookiePath);
      if (debugSession) {
