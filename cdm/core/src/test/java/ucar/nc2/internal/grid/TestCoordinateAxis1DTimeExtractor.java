...
public class TestCoordinateAxis1DTimeExtractor {
  ...
  public void testNumericValue() {
    ...
    assertThat(extract.timeHelper).isEqualTo(
            TimeHelper.factory(units, new AttributeContainerMutable("")));
  ...
  public void testStringValue() {
    ...
    VariableDS.Builder<?> vdsBuilder = VariableDS.builder().setName("name").setDataType(DataType.STRING)
            .setUnits(units)
            .setDesc("desc").setEnhanceMode(NetcdfDataset.getEnhanceAll())
            .addAttribute(new Attribute("missing_value", 0.0f))
            .setParentGroupBuilder(parent).setDimensionsByName("dim1")
  ...
  public void testCharValue() {
    ...
    VariableDS.Builder<?> vdsBuilder = VariableDS.builder().setName("name").setDataType(DataType.CHAR)
            .setUnits(units)
            .setDesc("desc").setEnhanceMode(NetcdfDataset.getEnhanceAll())
            .addAttribute(new Attribute("missing_value", 0.0f))
            .setParentGroupBuilder(parent).setDimensionsByName("dim1 " + nchars)
            .setSourceData(values);
    parent.addVariable(vdsBuilder);
