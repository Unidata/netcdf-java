...
public class TestGridAxis1D {
  ...
  public void testFromVariableDS() {
    ...
    VariableDS.Builder<?> vdsBuilder =
        VariableDS.builder().setName("name").setDataType(DataType.FLOAT).setUnits("unit").setAutoGen(0.0, 10.0)
    ...
    GridAxis1D.Builder<?> builder = GridAxis1D.builder(vds).setAxisType(AxisType.Ensemble);
    try {
    ...
    GridAxis1D axis1D = builder.build();
    ...
    assertThat(axis1D.getCoordEdgeMinMax()).isEqualTo(MinMax.create(-5.0, 65));
  ...
  public void testRegularPoint() {
    GridAxis1D.Builder<?> builder = GridAxis1D.builder().setAxisType(AxisType.GeoX).setName("name").setUnits("unit")
