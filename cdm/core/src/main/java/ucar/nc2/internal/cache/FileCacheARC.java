...
public class FileCacheARC implements FileCacheIF {
  ...
  protected final ConcurrentSkipListMap<CacheElement, CacheElement> shadowCache; // this is used to decide which file to
                                                                                 // release
                                                                                 // this can be biggger than the cache,
                                                                                 // eg ARC uses 2c, where c = cache size
  protected final ConcurrentHashMap<Object, CacheElement> cache; // collection of all unique files in the cache, keyed
                                                                 // by hashKey, typically filename
                                                                 // cache has open file handle / object in memory
  protected final ConcurrentHashMap<Integer, CacheElement.CacheFile> files; // collection of all files in the cache,
                                                                            // keyed by FileCacheable hashcode (object
                                                                            // id)
  ...
  private FileCacheable acquireCacheOnly(Object hashKey) {
    ...
    if (want.ncfile != null) {
      ...
      if (changed) { // underlying file was modified
        if (cacheLog.isDebugEnabled())
          cacheLog.debug("FileCacheARC " + name + ": acquire from cache " + hashKey + " " + want.ncfile.getLocation()
              + " was changed; discard");
  ...
  private CacheElement updateInCache(CacheElement elem) {
    ...
    elem.updateAccessed();
    CacheElement prev = shadowCache.put(elem, elem); // faster if we could just insert at the top of the list. maybe we
  ...
  public boolean release(FileCacheable ncfile) throws IOException {
    ...
    if (file != null) {
      if (!file.isLocked.get()) {
        ...
        cacheLog.warn(
            "FileCacheARC " + name + " release " + ncfile.getLocation() + " not locked; hash= " + ncfile.hashCode(), e);
      }
      // file.lastAccessed = System.currentTimeMillis();
      // file.countAccessed++;
      file.isLocked.set(false);
      if (cacheLog.isDebugEnabled())
        cacheLog.debug("FileCacheARC " + name + " release " + ncfile.getLocation() + "; hash= " + ncfile.hashCode());
      if (debugPrint)
        System.out.println("  FileCacheARC " + name + " release " + ncfile.getLocation());
      return true;
    }
    return false;
  ...
  class CacheElement {
    ...
    public String toString() {
      return hashKey + " count=" + list.size() + " countAccessed=" + countAccessed + " lastAccessed="
          + new Date(getLastAccessed());
    }
    ...
    class CacheFile implements Comparable<CacheFile> {
      ...
      private CacheFile(FileCacheable ncfile) {
        ...
        if (cacheLog.isDebugEnabled())
          cacheLog.debug("FileCacheARC " + name + " add to cache " + hashKey);
        if (debugPrint)
          System.out.println("  FileCacheARC " + name + " add to cache " + hashKey);
      }
      ...
      void close() {
        ...
        if (cacheLog.isDebugEnabled())
          cacheLog.debug("FileCacheARC " + name + " remove " + ncfile.getLocation());
        if (debugPrint)
          System.out.println("  FileCacheARC " + name + " remove " + ncfile.getLocation());
        ncfile = null;
      }
  ...
  public void showStats(Formatter format) {
    format.format("  hits= %d miss= %d nfiles= %d elems= %d shadow=%d%n", hits.get(), miss.get(), files.size(),
        cache.values().size(), shadowCache.size());
  }
