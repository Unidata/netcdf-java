...
public class Time2DCoordSys {
  ...
  public Optional<List<CoverageCoordAxis>> subset(SubsetParams params, AtomicBoolean isConstantForcast,
     ...
     * .subset(null, CoverageCoordAxis.Spacing.irregularPoint, ncoords, runValues); // LOOK check for regular (in
     * CovCoordAxis ?)
     ...
     * .subset(runAxisSubset2.getName(), CoverageCoordAxis.Spacing.irregularPoint, ncoords, offsetValues); // aux coord
     * (LOOK interval) ??
     ...
     * 
     * // nothing needed for CF, the run coordinate acts as the CF time independent coord. timeOffset is aux,
     * forecastTime is scalar
