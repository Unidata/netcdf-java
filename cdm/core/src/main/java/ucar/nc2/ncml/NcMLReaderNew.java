...
public class NcMLReaderNew {
   ...
   * @throws IOException on read error, or bad referencedDatasetUri URI
   */
  private NetcdfDataset readNcML(String ncmlLocation, @Nullable String referencedDatasetUri, Element netcdfElem,
    ...
    if (referencedDatasetUri != null) {
      if (iospS != null) {
        ...
        this.refFile = NetcdfDataset.openFile(durl, buffer_size, cancelTask, null);
      }
    }
  ...
  private void readNetcdf(String ncmlLocation, NetcdfDataset.Builder builder, Element netcdfElem, @Nullable CancelTask cancelTask) throws IOException {
    ...
    /* LOOK ignore aggregation for now
    ...
    Group.Builder rootGroup = readGroup(builder, null, null, netcdfElem);
    builder.setRootGroup(rootGroup);
  ...
  private Group.Builder readGroup(NetcdfDataset.Builder builder, @Nullable Group.Builder parent, @Nullable Group refParent, Element groupElem) {
    ...
    if (parent == null) {
      ...
      if (this.explicit || refGroup == null) { // all metadata is in the ncml, do not copy
        groupBuilder = Group.builder().setName("");
      } else { // already been copied
  ...
  private void readAtt(AttributeContainer dest, @Nullable AttributeContainer ref, Element attElem) {
    ...
    if (oldatt == null) { // new
      ...
      try {
        ...
        dest.addAttribute(Attribute.builder(name).setValues(values).build());
  ...
  private void readDim(Group.Builder groupBuilder, @Nullable Group refGroup, Element dimElem) {
    ...
    if (dim == null) { // nope - create it
      ...
      groupBuilder.addDimension(Dimension.builder(name, len)
          .setIsShared(isShared)
          .setIsUnlimited(isUnlimited)
          .setIsVariableLength(isVariableLength)
          .build());
  ...
  private Optional<Builder> readVariableExisting(Group.Builder groupBuilder, DataType dtype, Variable refv, Element varElem) {
    ...
    if (this.explicit) { // all metadata is in the ncml, do not copy
      ...
      v = (VariableDS.Builder) groupBuilder.findVariable(nameInFile).orElseThrow(()->new IllegalStateException("Cant find variable " + nameInFile));
    }
  ...
  private VariableDS.Builder readVariableNew(Group.Builder groupBuilder, DataType dtype, Element varElem) {
    ...
    VariableDS.Builder v = VariableDS.builder().setName(name).setDataType(dtype).addDimensions(varDims);
    Element valueElem = varElem.getChild("values", ncNS);
    if (valueElem != null) {
      readValues(v, dtype, varElem, valueElem);
    }
  ...
  private Optional<StructureDS.Builder> readStructureExisting(Group.Builder groupBuilder, DataType dtype, Structure refv, Element varElem) {
    ...
    if (this.explicit) { // all metadata is in the ncml, do not copy
      ...
      structBuilder = (StructureDS.Builder) groupBuilder.findVariable(nameInFile).orElseThrow(()->new IllegalStateException("Cant find variable " + nameInFile));
    }
   ...
   * @param varElem ncml variable element
   *
  private void readVariableNested(StructureDS.Builder parentS, Structure refStruct, Element varElem) {
    String name = varElem.getAttributeValue("name");
    if (name == null) {
      errlog.format("NcML Variable name is required (%s)%n", varElem);
      return;
    ...
    if (refv == null) { // new
      Variable.Builder nested = readVariableNew(ds, parentS.getParentGroup(), parentS, varElem);
      if (nested != null)
    ...
    if (dtype.isEnum()) {
      typedefS = varElem.getAttributeValue("typedef");
    }
  ...
  private void readValues(Variable.Builder v, DataType dtype, Element varElem, Element valuesElem) {
    try {
      ...
      int npts = (nptsS == null) ? (int) v.getShapeAsSection().getSize() : Integer.parseInt(nptsS);
