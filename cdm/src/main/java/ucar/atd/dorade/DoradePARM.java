...
public class DoradePARM extends DoradeDescriptor {
  ...
  public DoradePARM(RandomAccessFile file, boolean littleEndianData, DoradeRADD radd) throws DescriptorException {
   ...
   *         for bad data cells
   */
  public float[] getParamValues(DoradeRDAT rdat) throws DescriptorException {
    ...
    for (int cell = 0; cell < nCells; cell++) {
      switch (binaryFormat) {
        ...
        case DoradePARM.FORMAT_16BIT_INT:
          short sval = (svalues != null) ? svalues[cell] : grabShort(paramData, 2 * cell);
        ...
        default:
          throw new DescriptorException("bad binary format (" + binaryFormat + ")");
      }
    }
    return values;
  }
   ...
   */
  private short[] uncompressHRD(byte[] compressedData, int nCells) throws DescriptorException {
