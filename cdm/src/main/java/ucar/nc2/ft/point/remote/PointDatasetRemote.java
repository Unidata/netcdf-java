...
public class PointDatasetRemote extends PointDatasetImpl {
   ...
   * StructureMembers sm;
   * 
   * RemotePointFeatureIterator(PointStreamProto.PointFeatureCollection pfc, HttpMethod method, InputStream in) throws
   * IOException {
   * this.pfc = pfc;
   * this.method = method;
   * this.in = in;
   ...
   * 
   * MyPointFeature(EarthLocation location, double obsTime, double nomTime, DateUnit timeUnit,
   * PointStreamProto.PointFeature pfp) {
   ...
   * 
   * PointFeatureCollectionSubset(PointCollectionImpl from, LatLonRect filter_bb, DateRange filter_date) throws
   * IOException {
   * this.from = from;
   * 
   * if (filter_bb == null)
