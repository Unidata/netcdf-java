 ...
 * A factory will use the first PointFeature to get the StructureMembers object, and the DateUnits, and uses that for
 * all copies.
 ...
 * A object or array reference occupies one 32 bit word (4 bytes) on a 32 bit JVM or Davlik VM. A null takes the same
 * space as a reference.
 * (It has to, because a null has to fit in a reference-typed slot; i.e. instance field, local variable, etc.)
 * 
 * On the other hand, an object occupies a minimum of 2 32 bit words (8 bytes), and an array occupies a minimum of 3 32
 * bit words (12 bytes).
 * The actual size depends on the number and kinds of fields for an object, and on the number and kind of elements for
 * an array.
 * 
 ...
 * There is some fixed overhead per object. It's JVM-specific, but I usually estimate 40 bytes. Then you have to look at
 ...
 * Arrays follow the same rules; that is, it's an object reference so that takes 4 (or 8) bytes in your object, and then
 * its length multiplied by the size of its element.
 */
