...
import static ucar.nc2.internal.iosp.hdf5.zstd.Util.checkArgument;
...
class FiniteStateEntropy {
  ...
  public static int decompress(Table table, final Object inputBase, final long inputAddress, final long inputLimit,
      byte[] outputBuffer) {
  ...
  public static int compress(Object outputBase, long outputAddress, int outputSize, Object inputBase, long inputAddress,
      int inputSize, FseCompressionTable table) {
  ...
  public static int optimalTableLog(int maxTableLog, int inputSize, int maxSymbol) {
    ...
    result = Math.min(result, Util.highestBit((inputSize - 1)) - 2); // we may be able to reduce accuracy if input is
                                                                     // small
  ...
  private static int normalizeCounts2(short[] normalizedCounts, int tableLog, int[] counts, int total, int maxSymbol) {
    ...
    int lowThreshold = total >>> tableLog; // minimum count below which frequency in the normalized table is "too small"
                                           // (~ < 1)
