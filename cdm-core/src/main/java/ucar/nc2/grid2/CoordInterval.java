...
public abstract class CoordInterval {
  ...
  public abstract Number start();

  /** The ending value of the coordinate interval */
  public abstract Number end();

  /** Create an interval. */
  public static CoordInterval create(Number start, Number end) {
    return new AutoValue_CoordInterval(start, end);
  }

  /** The midpoint between start and end, cast to a double. */
  public double midpoint() {
    return (start().doubleValue() + end().doubleValue()) / 2;
  }

  /** Compare two intervals, with values cast to doubles, to within the given tolerence. */
  public boolean fuzzyEquals(CoordInterval other, double tol) {
    return DoubleMath.fuzzyEquals(start().doubleValue(), other.start().doubleValue(), tol)
        && DoubleMath.fuzzyEquals(end().doubleValue(), other.end().doubleValue(), tol);
  }
  ...
  public String toString(int ndecimals) {
    return Format.d(start().doubleValue(), ndecimals) + "-" + Format.d(end().doubleValue(), ndecimals);
  }
