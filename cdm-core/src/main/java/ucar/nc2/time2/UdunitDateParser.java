...
class UdunitDateParser {
   ...
   */
  static Optional<UdunitDateParser> parseUnitString(String udunitString) {
    ...
    Optional<OffsetDateTime> baseDate = parseUdunitIsoDate(refDate);

    return baseDate.map(d -> new UdunitDateParser(period, periodField, d, isCalendarField));
  }
   ...
   */
  static Optional<OffsetDateTime> parseUdunitIsoDate(String udunitDate) {
    ...
    try {
      ...
      if (zoneString != null) {
        ...
        if (!zoneString.isEmpty() && !zoneString.equalsIgnoreCase("Z") && !zoneString.equalsIgnoreCase("UTC")
          ...
          ZoneOffset zoneId = ZoneOffset.ofHoursMinutes(hourOffset, minuteOffset);
          OffsetDateTime dateTimeWithZone = OffsetDateTime.of(year, month, day, hour, minute, sec, nano, zoneId);
      ...
      // default time zone
      return Optional.of(OffsetDateTime.of(year, month, day, hour, minute, sec, nano, ZoneOffset.UTC));
  ...
  private UdunitDateParser(CalendarPeriod period, CalendarPeriod.Field periodField, OffsetDateTime baseDate,
